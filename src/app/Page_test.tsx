...
import { Plug, Trash2 } from 'lucide-react'; // Removed Camera icon as it's not in V0.0.1_clean base
...
import { useChartRenderer } from '../hooks/useChartRenderer'; // ADDED: Chart Renderer Hook

// UI Components (ensure ChartConfig types are handled if there are discrepancies)
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";
// import html2canvas from 'html2canvas'; // Now potentially only used inside useChartRenderer

// Types
// QueuedCommand, ChartDataPoint are from useCommandExecutor
// ChartConfig is handled by useChartRenderer internally using PageChartConfig from ui/chart
// DeviceProcedureParams is used by procedureListItems
...
const procedureListItems = [ // This could also be moved to a constants file
  {
    value: 'SwVin',
...
async function sendAndRead(
  ...
  // ... (sendAndRead implementation remains the same)
...
export default function Home() {
  ...
  // REMOVE Chart states: chartData, chartConfig, isChartReady - now in useChartRenderer
  ...
  const { generateCommandsFromProcedure } = useProcedure(viMin, viMax, nominalPower, tempProcedureInput); // Assuming useProcedure handles voNominal internally or doesn't need it based on V0.0.1_clean
  ...
  const {
    ...
    setCommands: setCommandsFromQueueManager,
    setCommandResponses: setCommandResponsesFromQueueManager,
  } = useCommandQueueManager({
    procedureListItems,
    deviceParams,
    generateCommandsFromProcedure,
    setPageResponse: setResponse,
    // setPageIsChartReady is now handled by useChartRenderer, so remove from here if not used by queue manager directly
    // setPageIsChartReady: setIsChartReady, // REMOVE/COMMENT if clearCommandQueue now calls setIsChartReady from useChartRenderer
    setPageCurrentProcedureName: setCurrentProcedureNameForExecutor,
  });

  // Effect to sync procedureInput from commandQueueManager to tempProcedureInput for useProcedure
  useEffect(() => {
    setTempProcedureInput(procedureInput);
  }, [procedureInput]);
  // Chart Renderer Hook Call
  const {
    // chartData, // Managed by this hook
    setChartData, // Expose setter for useCommandExecutor
    // chartConfig, // Managed by this hook
    setChartConfig, // Expose setter for useCommandExecutor
    // isChartReady, // Managed by this hook
    setIsChartReady, // Expose setter for useCommandExecutor
    ChartDisplayComponent, // The component to render the chart
    triggerChartCapture,    // The function to capture the chart
  } = useChartRenderer({
    currentProcedureName: currentProcedureNameForExecutor,
    converterModelForFilename: converterModel,
    procedureInputForFilename: procedureInput, // from useCommandQueueManager
    showEmailFormHandler: handleShowUserInfoForm, // from useEmailSender
    logUpdater: setResponse, // Pass the main response updater
  });

  // Command Executor Hook Call - now gets chart setters from useChartRenderer
  const { isExecuting, executeAllCommands } = useCommandExecutor({
    ...
    setChartDataInPage: setChartData,          // Setter from useChartRenderer
    setChartConfigInPage: setChartConfig,      // Setter from useChartRenderer
    setIsChartReadyInPage: setIsChartReady,    // Setter from useChartRenderer
  ...
  // REMOVE handleCaptureChart - its logic is now in triggerChartCapture from useChartRenderer
  ...
  // REMOVE chartLines definition - this logic is now inside useChartRenderer
  // const chartLines = ... ;
  return (
    <div className="flex flex-col items-center justify-start min-h-screen p-8 w-full">
      <h1 className="text-2xl font-bold mb-4">DC Converter Tester V0.3 (Chart Hook)</h1>

      <div className="w-full max-w-[80rem] space-y-4">
        {/* Converter Model Section - uses values from useDeviceParametersAndModel */}
        ...
        {/* Device Parameters Section - uses values from useDeviceParametersAndModel */}
        <div className="w-full p-4 border rounded-md shadow-sm">
        ...
        {/* Procedure and Command Entry - uses values from useCommandQueueManager */}
        ...
        {/* Command Queue Table - uses values from useCommandQueueManager */}
        {commands.length > 0 && (
        ...
        {/* Action Buttons - use handlers from respective hooks */}
        ...
        {/* Chart Display - Now rendered by useChartRenderer's component */}
        <ChartDisplayComponent />
        {/* Email Form Section - uses values from useEmailSender */}
        {showUserInfoForm && (
        ...
        {/* Port Activation Buttons - uses handlers from page.tsx / useComPort */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
